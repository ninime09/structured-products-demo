      ...
      <StageCard
        ...
        footer={
          <StageActionBar source="Approved Structure · 14:15">
            <ActionBtn label="Return for Modification" kind="ghost" allowed={['dealer', 'ps']} role={role} onClick={() => confirmThen({ key: 'returnRFQ', title: 'Return for Modification', summary: ['RFQ Package will be superseded', 'Return case to Product Specialist'], consequence: 'The approved structure will be reopened for modification.', confirmLabel: 'Return for modification', danger: true })} />
            <ActionBtn label="Accept Pricing Request" kind="primary" allowed={['dealer']} role={role} onClick={() => confirmThen({ key: 'acceptPricing', title: 'Accept Pricing Request', summary: [`FCN · 0700.HK · USD 1M · ${fv('Tenor')}`, `Strike ${fv('Strike')} · KI ${fv('Knock-In')}`, `Send to ${d.issuers.length} issuers`], consequence: 'The RFQ will be released to selected issuers and the case will enter market pricing.', confirmLabel: 'Accept pricing request' })} />
          </StageActionBar>
        }
      >
      ...
      <StageCard
        ...
        footer={
          <StageActionBar source="Live issuer quotes · 14:22">
            <ActionBtn label="Request Requote" kind="ghost" allowed={['dealer']} role={role} onClick={() => confirmThen({ key: 'requestRequote', title: 'Request Requote', summary: ['Request fresh quotes from all issuers', 'Current matrix becomes stale'], consequence: 'The market comparison will refresh when responses arrive.', confirmLabel: 'Request requote' })} />
            <ActionBtn label="Modify Structure" kind="secondary" allowed={['dealer', 'ps']} role={role} onClick={() => confirmThen({ key: 'modifyFromPricing', title: 'Modify Structure', summary: ['Return case to structure design', 'Current quote matrix becomes stale'], consequence: 'A new RFQ will be required after the structure is re-approved.', confirmLabel: 'Modify structure', danger: true })} />
            <ActionBtn label="Prepare Client Quote" kind="primary" allowed={['dealer']} role={role} aiRecommended onClick={() => confirmThen({ key: 'prepareClientQuote', title: 'Prepare Client Quote', summary: [`Selected ${best?.issuer}`, `Coupon ${best?.coupon?.toFixed(2)}% · Strike ${best?.strike} · KI ${best?.ki}`], consequence: 'AI will prepare a client-facing quote for RM review.', confirmLabel: 'Prepare client quote' })} />
          </StageActionBar>
        }
      >
    ...
    <div className="stage-workspace client-quote-workspace">
      ...
      <StageCard title="Client Quote Card" subtitle="RM review before external communication" status={<StatusBadge status={artifact.status} />} className="client-quote-stage-card"
        footer={<StageActionBar source="Quote Matrix · MS best comparable quote">
          <ActionBtn label="Request Updated Quote" kind="ghost" allowed={['rm', 'dealer']} role={role} onClick={() => confirmThen({ key: 'requestRequote', title: 'Request Updated Quote', summary: ['Return to market pricing', 'Request a fresh quote'], consequence: 'The current client quote will no longer be used.', confirmLabel: 'Request updated quote' })} />
          ...
          <ActionBtn label="Send to Client" kind="primary" allowed={['rm']} role={role} onClick={() => confirmThen({ key: 'sendClientQuote', title: 'Send Client Quote', summary: [`${d.issuer} · ${tv('Coupon')}`, `FCN 0700.HK · 6M · Strike ${tv('Strike')} · KI ${tv('Knock-In')}`], consequence: 'The reviewed message will be sent by RM and the case will wait for the client.', confirmLabel: 'Send to client' })} />
        </StageActionBar>}
      >
    ...
    <div className="stage-workspace instruction-workspace">
      ...
      <StageCard title="Client Instruction Detection" subtitle="Source Review" status={<Tag tone="warning">Pending RM confirmation</Tag>} className="instruction-stage-card">
        <div className="instruction-split">
          ...
          <Panel className="instruction-extract"><div className="split-panel-head"><span><Sparkles size={15} />AI extracted client instruction</span><Tag tone="success">Evidence linked</Tag></div><div className="intent-row"><span>Detected intent</span><strong>{d.intent}</strong><Tag tone="warning">{d.confidence} confidence</Tag></div><TermsTable rows={d.terms} /><div className="freshness-row"><div><Clock3 size={16} /><span><strong>Quote freshness</strong>Must remain valid at execution</span></div><Countdown until={validityUntil} now={now} /></div><div className="inline-stage-warning"><AlertTriangle size={15} /><span>Confirming creates a formal instruction record. Dealer must still obtain a live executable quote.</span></div><div className="instruction-actions"><ActionBtn label="Not an instruction" kind="ghost" allowed={['rm']} role={role} onClick={() => confirmThen({ key: 'rejectInstruction', title: 'Reject AI detection', summary: ['Mark this detection as invalid', 'Return to waiting for client'], consequence: 'No execution ticket will be created.', confirmLabel: 'Reject detection', danger: true })} /><Button variant="secondary">Ask client</Button><ActionBtn label="Confirm Instruction" kind="primary" allowed={['rm']} role={role} onClick={() => confirmThen({ key: 'confirmInstruction', title: 'Confirm Formal Client Instruction', summary: [`${iv('Issuer')} · ${iv('Notional')}`, `${iv('Product')} · ${iv('Strike / KI')}`, iv('Timing')], consequence: 'A formal instruction record and execution ticket draft will be created. Dealer will run a live quote check before execution.', confirmLabel: 'Confirm instruction' })} /></div></Panel>
        </div>
      </StageCard>
    </div>
  )
